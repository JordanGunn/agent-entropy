...
Stated as a prediction: if the Entropy of Comprehension is real, sessions anchored to a frozen intent record should show fewer late-stage goal reversals than sessions running on chat history alone, and the gap should widen with session count. The experiments below are how to measure it.
