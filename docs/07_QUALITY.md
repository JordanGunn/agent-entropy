# VI. What Messy Actually Means

The feeling described in `Section I` has names. **It has had them since 1999.** *The taxonomy is incomplete.*
...
What the rest of this chapter measures is not a fourth entropy. **It is the surface the other three are read from.** Where `Section IV`, `Section V`, and the expression gap describe the three entropies of the work — its *intent*, its *posture*, its *structure* — this chapter describes the **quality** of the artifact they leave behind, made measurable. And quality has exactly two surfaces:

```text
QUALITY = STRUCTURE + VOCABULARY
```
...
Quality is not a fourth entropy. **It is the surface the other three fossilise on.** A polluted goal, a degraded posture, a collapsed structure do not announce themselves — they *fossilise*: in a dependency graph drifting toward the Zone of Pain, in a vocabulary fragmenting into five words for one idea. The three rise **co-variantly**, and quality is the residue you can *see*. *This chapter is how you read the shadow the others cast.*
...
The methodology composes — Wille (1982) for clustering, Newman (2017) for stoplist principle, Zipf (1949) for distribution shape, Caprile & Tonella (2000) for identifier patterns, Bavota et al. for missing-class detection — into an analytical surface that *did not exist in this form*. Each constituent piece is decades old. **The composition, applied to agent-written code at this resolution, is new.**
